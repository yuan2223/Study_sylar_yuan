...
namespace yuan
{
    ...
    {
        m_previouseTime = yuan::GetCurrentMS();
    }
    ...
    uint64_t TimerManager::getNextTimer()
    {
        ...
        uint64_t now_ms = yuan::GetCurrentMS();
        if(now_ms >= next->m_ms)
        {
            return 0;
        }
        ...
        {
            return next->m_ms - now_ms;//要等待的时间
        }
    }
    ...
    {
        uint64_t now_ms = yuan::GetCurrentMS();
        std::vector<Timer::ptr> expired;            //已经超时的
        {
            RWMutexType::ReadLock lock(m_mutex);
            if(m_timers.empty())
            {
        ...
        auto it = rollover ? m_timers.end() : m_timers.lower_bound(now_timer);//大于等于
    ...
    void TimerManager::addTimer(Timer::ptr val,RWMutexType::WriteLock& lock)
    {
        ...
        bool at_front = (it == m_timers.begin()) && !m_tickled;//插入的定时器时间最小
        if(at_front)
        {
            m_tickled = true;
        }
